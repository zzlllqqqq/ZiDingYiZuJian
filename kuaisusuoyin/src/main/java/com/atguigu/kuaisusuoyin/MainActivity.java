...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initData();
        adapter = new CommonBaseAdapter<Person>(MainActivity.this, data, R.layout.item_main) {
            @Override
            public void convert(ViewHolder holder, int position) {
                Person person = data.get(position);
                holder.setText(R.id.tv_item_name, person.getName())
                        .setText(R.id.tv_item_word, person.getPinyin().substring(0, 1));
                if (position == 0) {
                    holder.setVisibility(R.id.tv_item_word, View.VISIBLE);
                } else {
                    String word = person.getPinyin().substring(0, 1);
                    String perWord = data.get(position - 1).getPinyin().substring(0, 1);
